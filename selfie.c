...
// symbols for assignment [bitwise-shift-compilation]
uint64_t SYM_SL = 33; // <<
uint64_t SYM_SR = 34; // >>

// symbols for assignment [bitwise-and-or-not]
uint64_t SYM_BIT_AND = 35; // &
uint64_t SYM_BIT_OR = 36;  // |
uint64_t SYM_BIT_NOT = 37; // ~

// [for-loop]]
uint64_t SYM_FOR = 38; // for

//[logical-and-or-not]
uint64_t SYM_LOG_AND = 39; // &&
uint64_t SYM_LOG_OR = 40;  // ||
uint64_t SYM_LOG_NOT = 41; // !

//[array]
uint64_t SYM_LBRACKET = 42; // [
uint64_t SYM_RBRACKET = 43; // ]

//[struct-declaration]
uint64_t SYM_STRUCT = 44; // struct

//[struct-execution]
uint64_t SYM_ARROW = 45; // ->

uint64_t SYM_END = 45; // Must be same value as last symbol known
...
uint64_t MAX_HEX_INTEGER_LENGTH = 16; // maximum number of characters in an hexadecimal unsigned integer
...
void init_scanner()
{
    SYMBOLS = smalloc((SYM_END + 1) * SIZEOFUINT64STAR);
    ...
    *(SYMBOLS + SYM_SR) = (uint64_t) ">>";
    ...
    //[bitwise-and-or-not]
    *(SYMBOLS + SYM_BIT_NOT) = (uint64_t) "~";
    *(SYMBOLS + SYM_BIT_AND) = (uint64_t) "&";
    *(SYMBOLS + SYM_BIT_OR) = (uint64_t) "|";
...
// |  5 | value            | VARIABLE: initial value, TYPE: size
// |  6 | address          | VARIABLE, BIGINT, STRING: offset, PROCEDURE: address
// |  7 | scope            | REG_GP (global), REG_S0 (local)
// |  8 | flags            | symbol flags, like volatile, const, array, struct, ...
// |  9 | array_dimensions | pointer to array dimension, single linked list
// | 10 | struct_fields    | pointer to struct field definitions, single linked list
// +---+-------------------+

// Array dimension descriptor entry
//[array-multidimensional]
// +---+---------+
// | 0 | next    | pointer to next entry
// | 1 | length  | length of array dimension
// +---+---------+
...
//[array]
uint64_t get_flags(uint64_t *entry) { return *(entry + 8); }
//[array-multidimensional]
uint64_t *get_array_dimension(uint64_t *entry) { return (uint64_t *)*(entry + 9); }
//[struct-declaration]
uint64_t *get_struct_field(uint64_t *entry) { return (uint64_t *)*(entry + 10); }
...
//[array]
void set_flags(uint64_t *entry, uint64_t flags) { *(entry + 8) = flags; }
//[array-multidimensional]
void set_array_dimension(uint64_t *entry, uint64_t *dimension) { *(entry + 9) = (uint64_t)dimension; }
//[struct-declaration]
void set_struct_field(uint64_t *entry, uint64_t *field) { *(entry + 10) = (uint64_t)field; }
...
// compile time types
uint64_t next_ctime_type = 5; // stores next free type number
...
//[struct-declaration]
// struct table
uint64_t STRUCT_TABLE = 4;

//[array] Name scopes
uint64_t GLOBAL = 1;
uint64_t LOCAL = 2;
uint64_t PARAMETER = 3;
//[struct-declaration]
uint64_t STRUCT = 4;
...
//[array] Symboltable flags
uint64_t SYMTAB_FLAG_ARRAY = 1;
uint64_t SYMTAB_FLAG_STRUCT = 2;
uint64_t SYMTAB_FLAG_POINTER = 4;
...
//[array]
uint64_t *compile_variable(uint64_t scope, char *variable, uint64_t type, uint64_t offset); // returns variable entry

//[array-multidimensional]
uint64_t compile_array(uint64_t *entry);

//[struct-declaration]
uint64_t *compile_struct(uint64_t scope, char *struct_name);
...
//[bitwise-shift-compilation]
uint64_t compile_shift(); // returns
...
//[array]
void emit_array_initialization(uint64_t *entry);
...
//[array]
uint64_t code_injection_start = 0;
uint64_t code_injection_end = 0;
uint64_t code_inject_only_once = 0;
...
//[bitwise-shift-execution]
void record_ssl_srl() { record_lui_addi_add_sub_mul_divu_remu_sltu_jal_jalr(); }
void undo_ssl_srl() { undo_lui_addi_add_sub_mul_divu_remu_sltu_load_jal_jalr(); }
void print_sll_srl_before() { print_add_sub_mul_divu_remu_sltu_before(); }
uint64_t print_sll_srl() { return print_add_sub_mul_divu_remu_sltu(); }
void print_sll_srl_after() { print_addi_add_sub_mul_divu_remu_sltu_after(); }
...
//[bitwise-and-or-not]
// XORI
void record_xori() { record_lui_addi_add_sub_mul_divu_remu_sltu_jal_jalr(); }
void undo_xori() { undo_lui_addi_add_sub_mul_divu_remu_sltu_load_jal_jalr(); }
void print_xori_after() { print_addi_add_sub_mul_divu_remu_sltu_after(); }
void print_xori_before() { print_addi_before(); }

uint64_t print_xori();
void do_xori();

// AND OR
void record_and_or() { record_lui_addi_add_sub_mul_divu_remu_sltu_jal_jalr(); }
void undo_and_or() { undo_lui_addi_add_sub_mul_divu_remu_sltu_load_jal_jalr(); }
void print_and_or_before() { print_add_sub_mul_divu_remu_sltu_before(); }
uint64_t print_and_or() { return print_add_sub_mul_divu_remu_sltu(); }
void print_and_or_after() { print_addi_add_sub_mul_divu_remu_sltu_after(); }
void do_and();
void do_or();
...
// is character a hexadecimal digit [hex-literal]
uint64_t is_hex_digit(char c)
{
    // ASCII codes for digits and characters are in a contiguous interval
    if (c >= '0')
        if (c <= '9')
            return 1;
...
// convert hex string to integer [hex-literal]
uint64_t atoi16(char *s)
{
    uint64_t i;
    uint64_t n;
...
void get_symbol()
{
    ...
    if (find_next_character() != CHAR_EOF)
    {
        if (symbol != SYM_DIVISION)
        {
            ...
            // extend less than and greater than [bitwise-shift-compilation]
                                               ...
                                               uint64_t line, uint64_t class, uint64_t type, uint64_t value, uint64_t address, uint64_t flags)
{
    ...
    //[struct-declaration]
    else if (table == STRUCT_TABLE)
    {
        set_scope(new_entry, REG_ZR);
        set_next_entry(new_entry, struct_symbol_table);
        struct_symbol_table = new_entry;
    }
    else
    {
        set_scope(new_entry, REG_GP);
...
//[array-multidimensional]
uint64_t compile_array(uint64_t *entry) // TRY OUT!!!
{
...
uint64_t *compile_struct_pointer(uint64_t scope, char *struct_type, uint64_t offset)
{
    ...
    if (symbol == SYM_IDENTIFIER)
    {
        ...
        // Create symbol table entry for pointer to struct

        // Find type corrsponding to struct name in symbol table
        entry = get_scoped_symbol_table_entry(struct_type, TYPE);
        if (entry)
        {
            type = get_type(entry);

            // This is pointer to struct
            flags = SYMTAB_FLAG_POINTER + SYMTAB_FLAG_STRUCT;
            if (scope == GLOBAL)
                entry = create_symbol_table_entry_with_flags(GLOBAL_TABLE, identifier, line_number, VARIABLE, type, 0, -offset, flags);
            else if (scope == LOCAL)
                entry = create_symbol_table_entry_with_flags(LOCAL_TABLE, identifier, line_number, VARIABLE, type, 0, -offset, flags);
            else if (scope == PARAMETER)
                entry = create_symbol_table_entry_with_flags(LOCAL_TABLE, identifier, line_number, VARIABLE, type, 0, offset, flags);
            else if (scope == STRUCT)
                entry = create_symbol_table_entry_with_flags(STRUCT_TABLE, identifier, line_number, VARIABLE, type, 0, offset, flags);
        }
        else
            syntax_error_message("Struct pointer declaration with undeclared struct type");
        get_symbol();
    }
    else
...
uint64_t *compile_struct(uint64_t scope, char *struct_name)
{
    ...
    {
        // Is struct declaration. We set size to 0.
        flags = SYMTAB_FLAG_STRUCT;
        if (scope == GLOBAL)
            entry = create_symbol_table_entry_with_flags(GLOBAL_TABLE, struct_name, line_number, TYPE, next_ctime_type, 0, 0, flags);
        else if (scope == LOCAL)
            entry = create_symbol_table_entry_with_flags(LOCAL_TABLE, struct_name, line_number, TYPE, next_ctime_type, 0, 0, flags);

        next_ctime_type = next_ctime_type + 1;
    }
    else if (symbol == SYM_LBRACE)
    {
        // Is struct definition

        // Lets see if struct already declared.
        entry = get_scoped_symbol_table_entry(struct_name, TYPE);
        if (entry == (uint64_t *)0)
        {
            // If not, create it in symbol table
            flags = SYMTAB_FLAG_STRUCT;
            if (scope == GLOBAL)
                entry = create_symbol_table_entry_with_flags(GLOBAL_TABLE, struct_name, line_number, TYPE, next_ctime_type, 0, 0, flags);
            else if (scope == LOCAL)
        ...
        // Clear symbol table for actual struct
        struct_symbol_table = (uint64_t *)0;
        size = 0;

        get_symbol();
        while (is_type() || is_struct())
        {
            if (is_type())
            {
                // Primitive type inside struct body
                type = compile_type();
                compile_variable(STRUCT_TABLE, identifier, type, size);
                size = size + WORDSIZE;
            ...
            {
                // Must be struct pointer inside struct body
                get_symbol();
                if (symbol == SYM_IDENTIFIER)
                {
                    ...
                    {
                        get_symbol();
                        compile_struct_pointer(STRUCT_TABLE, struct_type, size);
                        size = size + WORDSIZE;
                        get_expected_symbol(SYM_SEMICOLON);
                    }
                    else
        ...
        // We have body done, now set structs field definitions and size
        set_struct_field(entry, struct_symbol_table);
        set_value(entry, size);

        // struct definition must end with right brace
...
void compile_cstar()
{
    ...
    code_injection_start = code_size;
    array_length = 0;
        ...
        if (is_type())
        {
            ...
            if (symbol == SYM_IDENTIFIER)
            {
                ...
                if (symbol == SYM_LBRACKET)
                {
                    ...
                    // Because we do not know offest here, set it 0. Will be set to correct value later.
                    entry = compile_variable(GLOBAL, variable_or_procedure, type, 0);

                    // Compile array now and get size to set correct offset
                    array_length = compile_array(entry);

                    // Size of type = pointer + array fields
                    data_size = data_size + WORDSIZE + array_length * WORDSIZE;
                    offset = data_size;

                    set_address(entry, -offset);
                    set_flags(entry, get_flags(entry) | SYMTAB_FLAG_ARRAY);

                    emit_array_initialization(entry);
                ...
                else if (symbol == SYM_LPARENTHESIS)
                {
                    ...
                    if (code_inject_only_once == 0)
                    {
                        code_injection_end = code_size;
                        code_inject_only_once = 1;
                    }
                ...
                else
                {
                    ...
                    offset = data_size;
                    entry = compile_variable(GLOBAL, variable_or_procedure, type, offset);
        ...
        else if (symbol == SYM_STRUCT)
        {
            ...
            if (symbol == SYM_IDENTIFIER)
            {
                ...
                if (symbol == SYM_ASTERISK)
                {
                    ...
                    get_symbol();
                    entry = compile_struct_pointer(GLOBAL, struct_name, offset);
                    get_expected_symbol(SYM_SEMICOLON);
                }
                else
                {
                    // It is declaration or definition.
                    entry = compile_struct(GLOBAL, struct_name);
                    // struct definition and declaration must end with semicolon
                    get_expected_symbol(SYM_SEMICOLON);
                }
            }
            else
...
//[array]
uint64_t *compile_variable(uint64_t scope, char *variable, uint64_t type, uint64_t offset)
{
    uint64_t *entry;
    ...
    if (scope == GLOBAL)
    {
        ...
        if (entry == (uint64_t *)0)
        {
            ...
            entry = create_symbol_table_entry(GLOBAL_TABLE, variable, line_number, VARIABLE, type,
                                              0, -offset);
        }
        else
        {
    ...
    else if (scope == LOCAL)
    {
        ...
        entry = create_symbol_table_entry(LOCAL_TABLE, variable, line_number, VARIABLE, type, 0,
                                          -offset);
    }
    else if (scope == PARAMETER)
    {
    ...
    else if (scope == STRUCT)
    {
        ...
        // TODO: check if identifier has already been declared
        entry = create_symbol_table_entry(STRUCT_TABLE, variable, line_number, VARIABLE, type, 0,
                                          offset);
    }
    return entry;
}
...
//[struct-execution]
uint64_t *get_type_entry(uint64_t type)
{
    uint64_t *entry;
    uint64_t t;
    uint64_t i;

    // First search in local symbol table
    entry = local_symbol_table;
    while (entry != (uint64_t *)0)
    {
        if (get_class(entry) == TYPE)
        {
            t = get_type(entry);
            if (t == type)
                return entry;
    ...
    // Second search in global symbol table
    for (i = 0; i < HASH_TABLE_SIZE; i = i + 1)
    {
        entry = global_symbol_table + i;
        while (entry != (uint64_t *)0)
        {
            if (get_class(entry) == TYPE)
            {
...
void compile_assignment(char *variable)
{
    ...
    offset = 0;
    structure = 0;
    struct_type_entry = (uint64_t *)0;
    field_entry = (uint64_t *)0;
    ...
    if (variable != (char *)0)
    {
        if (symbol == SYM_LBRACKET)
        {
            ...
            array_desc = get_array_dimension(entry);
            ...
            while (symbol == SYM_LBRACKET)
            {
                get_symbol();
                compile_expression();
                // assert: allocated_temporaries == 3

                // Multiply array offset with all array dimensions below actual level
                talloc(); // t3
                // assert: allocated_temporaries == 4
                temp_desc = array_desc;
                while (temp_desc != (uint64_t *)0)
                {
                    layer_size = *(temp_desc + 1);
                    emit_addi(current_temporary(), REG_ZR, layer_size);
                    emit_mul(previous_temporary(), previous_temporary(), current_temporary());
                    // Get next descriptor entry
                    temp_desc = (uint64_t *)*temp_desc;
                }
                tfree(1);
                // assert: allocated_temporaries == 3
                ...
                // Prepare descriptor for next array level
                if (array_desc != (uint64_t *)0)
                    array_desc = (uint64_t *)*array_desc;

                get_expected_symbol(SYM_RBRACKET);
            }

            if (array_desc)
                syntax_error_message("Array not allowed on left side of assignment");

            // Multiply array offset by 8 to get address offset
            talloc();
            // assert: allocated_temporaries == 3
            emit_addi(current_temporary(), REG_ZR, 8);
            emit_mul(previous_temporary(), previous_temporary(), current_temporary());
            tfree(1);
            // assert: allocated_temporaries == 2
            emit_add(previous_temporary(), previous_temporary(), current_temporary());

            base = previous_temporary();
            offset = 0;
        ...
        else if (symbol == SYM_ARROW)
        {
            ...
            // Get symbol table entry
            entry = get_variable_entry(struct_name);
            base = get_scope(entry);
            type = get_type(entry);

            // load variable value (as address)
            ltype = load_variable(struct_name);
            // assert: allocated_temporaries == 1
            ...
            while (symbol == SYM_ARROW)
            {
                ...
                if (symbol == SYM_IDENTIFIER)
                {
                    ...
                    // Get corresponding struct type in symbol table for this struct
                    struct_type_entry = get_type_entry(type);
                    struct_symbol_table = get_struct_field(struct_type_entry);

                    // Search field entry
                    field_entry = search_symbol_table(struct_symbol_table, field_name, VARIABLE);
                    field_offset = get_address(field_entry);
                    type = get_type(field_entry);

                    // Calculate address of actual field
                    emit_addi(current_temporary(), current_temporary(), field_offset);

                    get_symbol();
                    // If we have a pointer to struct and another arrow, dereference field
                    if (symbol == SYM_ARROW)
                    {
                        if (get_flags(field_entry) & SYMTAB_FLAG_POINTER)
                            if (get_flags(field_entry) & SYMTAB_FLAG_STRUCT)
                                emit_load(current_temporary(), current_temporary(), 0);
                            else
                                syntax_error_message("Pointer must point to struct");
                        else
                            syntax_error_message("Must be pointer to struct");
                    }
                }
                else
                    syntax_error_expected_symbol(SYM_IDENTIFIER);
            ...
            // Set to primitive pointer because selfie can not handle other types
            if (get_flags(field_entry) & SYMTAB_FLAG_POINTER)
                type = UINT64STAR_T;
        ...
        else
        {
            ...
            if (get_flags(entry) == SYMTAB_FLAG_ARRAY)
                syntax_error_message("Array not allowed on left side of assignment");

            //[struct-execution]
            if (get_flags(entry) & SYMTAB_FLAG_STRUCT)
            {
                // We have struct
                // Left side of assignment must be struct pointer
                if (get_flags(entry) & SYMTAB_FLAG_POINTER)
                {
                    // We have struct pointer
                    base = get_scope(entry);
                    ...
                    syntax_error_message("Struct not allowed on left side of assignment");
            }
            else
            {
    ...
    else
    {
        ...
        if (symbol == SYM_IDENTIFIER)
        {
            ...
            // If it is array, interpret it as pointer
            entry = get_variable_entry(variable);
            if (get_flags(entry) & SYMTAB_FLAG_ARRAY)
            {
                if (get_type(entry) == UINT64_T)
                    ltype = UINT64STAR_T;
            }
        }
        else if (symbol == SYM_LPARENTHESIS)
...
{
    uint64_t ltype;
    uint64_t instr_addr_of_jmp_to_end;

    instr_addr_of_jmp_to_end = 0;
    ...
    while (is_log_or())
    {
        ...
        emit_beq(current_temporary(), REG_ZR, 3 * INSTRUCTIONSIZE);
        // if left side of or is true, we have done ...
        emit_addi(current_temporary(), REG_ZR, 1);
        // ... so jump to the end of or. Jump to zero until we know the correct address
        instr_addr_of_jmp_to_end = code_size;
        emit_jal(REG_ZR, 0);

        compile_logical_and();

        // assert: allocated_temporaries == n + 2
        // if right side of or is also false return false
        emit_beq(current_temporary(), REG_ZR, 2 * INSTRUCTIONSIZE);
        emit_addi(previous_temporary(), REG_ZR, 1);

        // now we know lentgth of body, so correct the branch lentgth
        if (instr_addr_of_jmp_to_end != 0)
            fixup_relative_JFormat(instr_addr_of_jmp_to_end, code_size);
    ...
    // type of bitwise or expression is grammar attribute
    return ltype;
}
...
uint64_t compile_logical_and()
{
    ...
    while (is_log_and())
    {
        ...
        // jump to zero until we know the correct address
        instr_addr_of_jmp_to_end = code_size;
        ...
        // now we know lentgth of body, so correct the branch lentgth
        if (instr_addr_of_jmp_to_end != 0)
            fixup_relative_BFormat(instr_addr_of_jmp_to_end);
    ...
    // type of bitwise and expression is grammar attribute
    return ltype;
}
...
uint64_t compile_factor()
{
    ...
    // optional: "~" [bitwise-and-or-not]
    if (symbol == SYM_BIT_NOT)
    {
        not = 1;
        get_symbol();
    }
    ...
    // optional: "!" [logical-and-or-not]
    if (symbol == SYM_LOG_NOT)
    {
        log_not = 1;
        get_symbol();
    }
    ...
    else if (symbol == SYM_IDENTIFIER)
    {
        ...
        if (symbol == SYM_ARROW)
        {
            ...
            // variable is struct pointer
            dereference = 0;
            struct_name = variable_or_procedure;
            ...
            // load variable value (as address)
            type = load_variable(struct_name);
            // assert: allocated_temporaries == n + 1
        ...
        else if (symbol == SYM_LBRACKET)
        {
            ...
            while (symbol == SYM_LBRACKET)
            {
                get_symbol();
                compile_expression();
                // assert: allocated_temporaries == n + 3

                // Multiply array offset with all array dimensions below actual level
                talloc();
                // assert: allocated_temporaries == n + 4
                temp_desc = array_desc;
                while (temp_desc != (uint64_t *)0)
                {
                    layer_size = *(temp_desc + 1);
            ...
            // Array dereferenced to deepest layer
            if (array_desc == (uint64_t *)0)
            {
                // assert: allocated_temporaries == n + 1
                tfree(1);
            ...
            // Array not dereferenced to deepest layer, so interpret it as pointer
            else
            {
                // If it is array, interpret it as pointer
                entry = get_variable_entry(variable_or_procedure);
...
void load_big_integer(uint64_t value)
{
    ...
    if (entry == (uint64_t *)0)
    {
        ...
        entry = create_symbol_table_entry(GLOBAL_TABLE, integer, line_number, BIGINT, UINT64_T,
                                          value, -data_size);
...
void load_string()
{
    ...
    if (entry == (uint64_t *)0)
    {
        ...
        entry = create_symbol_table_entry(GLOBAL_TABLE, string, line_number, STRING, UINT64STAR_T,
                                          0, -data_size);
...
void compile_for()
{
    uint64_t code_addr_for_condition;
    uint64_t code_addr_for_assignment;

    uint64_t instr_addr_of_jmp_to_end;
    uint64_t instr_addr_of_jmp_to_body;

    char *variable;

    // assert: allocated_temporaries == 0

    code_addr_for_condition = 0;
    code_addr_for_assignment = 0;

    instr_addr_of_jmp_to_end = 0;
    instr_addr_of_jmp_to_body = 0;
    ...
    if (symbol == SYM_FOR)
    {
        ...
        if (symbol == SYM_LPARENTHESIS)
        {
            ...
            // Is there a for assignment? Then ...
            if (symbol != SYM_SEMICOLON)
            {
                // ... compile optional left assignment
                if (symbol == SYM_IDENTIFIER)
                {
                    variable = identifier;
                    get_symbol();
            ...
            // Is there a for condition? ...
            code_addr_for_condition = code_size;
            if (symbol != SYM_SEMICOLON)
            {
                // ... Then compile optional condition
                compile_logical_or();

                // if the "for" condition is false jump to for end
                // the target address is still unknown, using 0 for now
                // but remember the address of this beq instruction to correct it later
                instr_addr_of_jmp_to_end = code_size;
                emit_beq(current_temporary(), REG_ZR, 0);

                // jump to body, but the target address is still unknown, using 0 for now
                // remember the address of this beq instruction to correct it later
                instr_addr_of_jmp_to_body = code_size;
            ...
            {
                // ... Else loop forever. Branch to body, but never to end of for loop
                instr_addr_of_jmp_to_body = code_size;
                emit_beq(REG_ZR, REG_ZR, 0);
            }
            get_required_symbol(SYM_SEMICOLON);

            // compile optional right assignment
            code_addr_for_assignment = code_size;
            if (symbol != SYM_RPARENTHESIS)
            {
                if (symbol == SYM_IDENTIFIER)
                {
            ...
            // Jump to for condition after assignment execution
            emit_jal(REG_ZR, code_addr_for_condition - code_size);

            // now we know lentgth of left assignment, so correct the branch lentgth
            if (instr_addr_of_jmp_to_body != 0)
                fixup_relative_BFormat(instr_addr_of_jmp_to_body);

            // Head of for is done, so compile body now
            if (symbol == SYM_LBRACE)
            {
                // zero or more statements: "{" { statement } "}"
                get_symbol();
    ...
    // After executing the body jump to right assignment of for loop
    emit_jal(REG_ZR, code_addr_for_assignment - code_size);
...
void compile_procedure(char *procedure, uint64_t type)
{
    ...
    uint64_t code_addr_locals_init_jal;
    uint64_t code_addr_locals_init_begin;
    uint64_t code_addr_locals_init_end;
                ...
                if (is_type())
                {
                    ...
                    if (symbol == SYM_IDENTIFIER)
                    {
                        ...
                        if (symbol == SYM_LBRACKET)
                        {
                            ...
                            offset = 2 * WORDSIZE;
                            ventry = compile_variable(PARAMETER, variable, vtype, offset);
                            array_length = compile_array(ventry);
                        }
                        else
                        {
                            // 2 * WORDSIZE offset to skip frame pointer and link
                            ...
                            // since actual parameters are pushed onto stack in reverse
                            offset = 2 * WORDSIZE;
                            compile_variable(PARAMETER, variable, vtype, offset);
                        }
                ...
                else if (is_struct())
                {
                    ...
                    if (symbol == SYM_IDENTIFIER)
                    {
                        ...
                        if (symbol == SYM_ASTERISK)
                        {
                            ...
                            get_symbol();
                            compile_struct_pointer(PARAMETER, struct_type, offset);
                            number_of_formal_parameters = 1;
                        }
                        else
                            syntax_error_expected_symbol(SYM_ASTERISK);
                    ...
                    else if (is_type() || is_struct())
                    {
                        if (is_type())
                        {
                            ...
                            { // Store to look ahead
                                ...
                                {
                                    //[[array-multidimensional]
                                    offset = 2 * WORDSIZE + number_of_formal_parameters * WORDSIZE;
                                    ventry = compile_variable(PARAMETER, variable, vtype, offset);
                                    compile_array(ventry);
                                }
                                else
                                {
                                    offset = 2 * WORDSIZE + number_of_formal_parameters * WORDSIZE;
                        ...
                        syntax_error_message("Expected type or struct as function parameter!");
                }
                get_expected_symbol(SYM_RPARENTHESIS);
            }
            else
    ...
    if (entry == (uint64_t *)0)
        ...
        entry = create_symbol_table_entry(GLOBAL_TABLE, procedure, line_number, PROCEDURE, type,
                                          number_of_formal_parameters, 0);
    ...
    else if (symbol == SYM_LBRACE)
    {
        ...
        //[array]
        // Jump to preamble before executing code regarding to locals initialization

        /* IMPORTANT
           NOP before jal is mandatory at this place, because otherwise selfie will not compile main function
           correctly. There is a condition during bootstrap which tests if the first instruction in a function
           is a jal, which is used to indicate, that a function is declared but not defined.
        */
        emit_nop();                            // This nop is mandatory
        code_addr_locals_init_jal = code_size; // remember this jal instruction address
        // emit_beq(REG_ZR, REG_ZR, 0);            // Use 0 as offset because we do not know the correct address now
        emit_jal(REG_ZR, 0);
        code_addr_locals_init_begin = code_size; // remember start address of locals init
        while (is_type() || is_struct())
        {
            if (is_type())
            {
                ...
                if (symbol == SYM_IDENTIFIER)
                {
                    ...
                    // Jump here after emitting procedures preamble
                    ...
                    if (symbol == SYM_LBRACKET)
                    {
                        ...
                        // Because we do not know offest here, set it 0. Will be set to correct value later.
                        ventry = compile_variable(LOCAL, variable, vtype, 0);

                        array_length = compile_array(ventry);

                        // Size of type = pointer + array fields
                        number_of_local_variable_bytes = number_of_local_variable_bytes + WORDSIZE + array_length * WORDSIZE;
                        offset = number_of_local_variable_bytes;

                        set_address(ventry, -offset);
                        set_flags(ventry, get_flags(ventry) | SYMTAB_FLAG_ARRAY);

                        emit_array_initialization(ventry);
                    }
                    else
                    {
                        number_of_local_variable_bytes = number_of_local_variable_bytes + WORDSIZE;
                        offset = number_of_local_variable_bytes;
                        compile_variable(LOCAL, variable, vtype, offset);
                    }
                }
                else
                {
            ...
            else if (is_struct())
            {
                ...
                if (symbol == SYM_IDENTIFIER)
                {
                    ...
                    if (symbol == SYM_ASTERISK)
                    {
                        ...
                        get_symbol();
                        ventry = compile_struct_pointer(LOCAL, struct_type, offset);
                        get_expected_symbol(SYM_SEMICOLON);
                    }
                    else
                        syntax_error_expected_symbol(SYM_ASTERISK);
        ...
        /*********************************************
            Emit prolog for procedure and init locals
        */
        // After locals init is done, jump to procedures body
        code_addr_locals_init_end = code_size;
        emit_jal(REG_ZR, 0); // Offset is corrected if we know address to jump to

        // Now we know the address so set jal offset
        fixup_relative_JFormat(code_addr_locals_init_jal, code_size);
        procedure_prologue(number_of_local_variable_bytes);

        // Jump to locals initialization after prolog
        emit_jal(REG_ZR, code_addr_locals_init_begin - code_size);

        // Jump here after locals initialization
        fixup_relative_JFormat(code_addr_locals_init_end, code_size);

        /*
            Procedures prolog and init done
        *********************************************/
        ...
        // Inject data initialisation code for globals
        if (string_compare(procedure, main_name))
        {
            while (code_injection_start < code_injection_end)
            {
                instruction = load_instruction(code_injection_start);
                emit_instruction(instruction);
                code_injection_start = code_injection_start + INSTRUCTIONSIZE;
            }
        }
...
uint64_t procedure_call(uint64_t *entry, char *procedure, uint64_t number_of_actual_parameters)
{
    ...
    if (entry == (uint64_t *)0)
    {
        ...
        create_symbol_table_entry(GLOBAL_TABLE, procedure, line_number, PROCEDURE, type,
                                  number_of_actual_parameters, code_size);
...
void selfie_compile()
{
    ...
    while (link)
    {
        ...
        else
        {
            ...
            printf("%s: %lu characters read in %lu lines and %lu comments\n", selfie_name,
                   number_of_read_characters, line_number, number_of_comments);
            ...
            printf("%s: %lu global variables, %lu types, %lu procedures, %lu string literals\n", selfie_name,
                   number_of_global_variables, number_of_types, number_of_procedures, number_of_strings);

            printf("%s: %lu calls, %lu assignments, %lu while, %lu if, %lu return, %lu for\n",
                   selfie_name, number_of_calls, number_of_assignments, number_of_while,
                   number_of_if, number_of_return, number_of_for);
    ...
    printf("%s: symbol table search time was %lu iterations on average and %lu in total\n",
           selfie_name, total_search_time / number_of_searches, total_search_time);

    printf("%s: %lu bytes generated with %lu instructions and %lu bytes of data\n", selfie_name,
           code_size + data_size, code_size / INSTRUCTIONSIZE, data_size);
...
void read_register(uint64_t reg) { read_register_wrap(reg, 1); }
...
void write_register(uint64_t reg) { write_register_wrap(reg, 1); }
...
void check_immediate_range(uint64_t immediate, uint64_t bits)
{
    if (is_signed_integer(immediate, bits) == 0)
    {
        ...
        printf("%ld expected between %ld and %ld\n", immediate, -two_to_the_power_of(bits - 1),
...
uint64_t encode_r_format(uint64_t funct7, uint64_t rs2, uint64_t rs1, uint64_t funct3, uint64_t rd,
                         uint64_t opcode)
{
    // assert: 0 <= funct7 < 2^7
    // assert: 0 <= rs2 < 2^5
    ...
    return left_shift(
               left_shift(left_shift(left_shift(left_shift(funct7, 5) + rs2, 5) + rs1, 3) + funct3,
                          5) +
                   rd,
               7) +
           opcode;
...
uint64_t get_funct7(uint64_t instruction) { return get_bits(instruction, 25, 7); }
...
uint64_t get_funct3(uint64_t instruction) { return get_bits(instruction, 12, 3); }

uint64_t get_rd(uint64_t instruction) { return get_bits(instruction, 7, 5); }

uint64_t get_opcode(uint64_t instruction) { return get_bits(instruction, 0, 7); }
...
uint64_t encode_s_format(uint64_t immediate, uint64_t rs2, uint64_t rs1, uint64_t funct3,
                         uint64_t opcode)
{
    // assert: -2^11 <= immediate < 2^11
    // assert: 0 <= rs2 < 2^5
    ...
    return left_shift(
               left_shift(left_shift(left_shift(left_shift(imm1, 5) + rs2, 5) + rs1, 3) + funct3,
                          5) +
                   imm2,
               7) +
           opcode;
...
uint64_t encode_b_format(uint64_t immediate, uint64_t rs2, uint64_t rs1, uint64_t funct3,
                         uint64_t opcode)
{
    ...
    return left_shift(
               left_shift(
                   left_shift(
                       left_shift(left_shift(left_shift(left_shift(imm1, 6) + imm2, 5) + rs2, 5) +
                                      rs1,
                                  3) +
                           funct3,
                       4) +
...
uint64_t get_immediate_b_format(uint64_t instruction)
{
    ...
    return sign_extend(
        left_shift(left_shift(left_shift(left_shift(imm1, 1) + imm4, 6) + imm2, 4) + imm3, 1), 13);
}
...
uint64_t encode_j_format(uint64_t immediate, uint64_t rd, uint64_t opcode)
{
    ...
    return left_shift(
               left_shift(left_shift(left_shift(left_shift(imm1, 10) + imm2, 1) + imm3, 8) + imm4,
                          5) +
                   rd,
               7) +
           opcode;
...
uint64_t get_immediate_j_format(uint64_t instruction)
{
    ...
    // reassemble immediate and add trailing zero
    return sign_extend(
        left_shift(left_shift(left_shift(left_shift(imm1, 8) + imm4, 1) + imm3, 10) + imm2, 1), 21);
}
...
uint64_t get_total_number_of_instructions()
{
    return ic_lui + ic_addi + ic_add + ic_sub + ic_mul + ic_divu + ic_remu + ic_sltu + ic_load +
           ic_store + ic_beq + ic_jal + ic_jalr + ic_ecall + ic_sll + ic_srl + ic_xori + ic_and +
           ic_or;
}

uint64_t get_total_number_of_nops()
{
    return nopc_lui + nopc_addi + nopc_add + nopc_sub + nopc_mul + nopc_divu + nopc_remu +
           nopc_sltu + nopc_load + nopc_store + nopc_beq + nopc_jal + nopc_jalr + nopc_sll +
           nopc_srl + nopc_xori + nopc_and + nopc_or;
}

void print_instruction_counter(uint64_t counter, uint64_t ins)
{
    printf("%s: %lu(%lu.%.2lu%%)", get_mnemonic(ins), counter,
...
uint64_t get_low_word(uint64_t word) { return get_bits(word, 0, SINGLEWORDSIZEINBITS); }
    ...
    if (IS64BITSYSTEM)
    {
        ...
        if (waddr % SIZEOFUINT64 == 0)
            ...
            *(memory + waddr / SIZEOFUINT64) =
                left_shift(load_word(memory, waddr + SINGLEWORDSIZE, is_double_word),
...
uint64_t load_instruction(uint64_t caddr) { return load_word(code_binary, caddr, 0); }
...
uint64_t encode_nop() { return encode_i_format(0, REG_ZR, F3_NOP, REG_ZR, OP_IMM); }
...
{
    uint64_t base;
    uint64_t pointer_addr_temp;
    uint64_t array_addr_temp;

    // Store address of first array element as array pointer value
    base = get_scope(entry);
    // load offset into temp reg
    load_integer(get_address(entry));
    pointer_addr_temp = current_temporary();
    // add base to get pointer's address
    emit_add(pointer_addr_temp, pointer_addr_temp, base);

    // add 8 to pointers address, to get address of array start
    talloc();
    array_addr_temp = current_temporary();
    emit_addi(array_addr_temp, REG_ZR, 8);
    emit_add(array_addr_temp, array_addr_temp, pointer_addr_temp);

    // now store array address as value of pointer
    emit_store(pointer_addr_temp, 0, array_addr_temp);
    // free temps
    tfree(2);
}
...
void fixup_relative_BFormat(uint64_t from_address)
{
    ...
    store_instruction(from_address, encode_b_format(code_size - from_address, get_rs2(instruction),
                                                    get_rs1(instruction), get_funct3(instruction),
...
void fixup_relative_JFormat(uint64_t from_address, uint64_t to_address)
{
    ...
    store_instruction(from_address, encode_j_format(to_address - from_address, get_rd(instruction),
                                                    get_opcode(instruction)));
}
...
void selfie_output(char *filename)
{
    ...
    printf(
        "%s: %lu bytes with %lu %lu-bit RISC-U instructions and %lu bytes of data written into %s\n ",
        selfie_name, ELF_HEADER_SIZE + code_size + data_size, code_size / INSTRUCTIONSIZE,
        WORDSIZEINBITS, data_size, binary_name);
}
    ...
    if (number_of_read_bytes == ELF_HEADER_SIZE)
    {
        if (validate_elf_header(ELF_header))
        {
            ...
            number_of_read_bytes =
                sign_extend(read(fd, code_binary, code_size_with_padding), SYSCALL_BITWIDTH);
            ...
            if (number_of_read_bytes == code_size_with_padding)
            {
                ...
                if (number_of_read_bytes == data_size)
                {
                    ...
                    {
                        printf(
                            "%s: %lu bytes with %lu %lu-bit RISC-U instructions and %lu bytes of data loaded from %s\n",
...
void implement_read(uint64_t *context)
{
    ...
    if (debug_read)
        printf(
            "%s: trying to read %lu bytes from file with descriptor %lu into buffer at virtual address 0x%08lX\n",
            selfie_name, size, fd, (uint64_t)vbuffer);
        ...
        if (is_virtual_address_valid(vbuffer, WORDSIZE))
            if (is_data_stack_heap_address(context, vbuffer))
                ...
                else
                {
                    ...
                    printf(
                        "%s: reading into virtual address 0x%08lX failed because the address is unmapped\n",
            ...
            else
            {
                ...
                printf(
                    "%s: reading into virtual address 0x%08lX failed because the address is in an invalid segment\n",
                    selfie_name, (uint64_t)vbuffer);
            }
        else
        {
            ...
            printf(
                "%s: reading into virtual address 0x%08lX failed because the address is invalid\n",
                selfie_name, (uint64_t)vbuffer);
        }
    }
...
void implement_write(uint64_t *context)
{
    ...
    if (debug_write)
        printf(
            "%s: trying to write %lu bytes from buffer at virtual address 0x%08lX into file with descriptor %lu\n",
            selfie_name, size, (uint64_t)vbuffer, fd);
        ...
        if (is_virtual_address_valid(vbuffer, WORDSIZE))
            if (is_data_stack_heap_address(context, vbuffer))
                ...
                else
                {
                    ...
                    printf(
                        "%s: writing from virtual address 0x%08lX failed because the address is unmapped\n",
                        selfie_name, (uint64_t)vbuffer);
                }
            else
            {
                ...
                printf(
                    "%s: writing from virtual address 0x%08lX failed because the address is in an invalid segment\n",
                    selfie_name, (uint64_t)vbuffer);
            }
        else
        {
            ...
            printf(
                "%s: writing from virtual address 0x%08lX failed because the address is invalid\n",
                selfie_name, (uint64_t)vbuffer);
        }
    }
    ...
    while (i < MAX_FILENAME_LENGTH)
    {
        if (is_virtual_address_valid(vaddr, WORDSIZE))
            if (is_data_stack_heap_address(context, vaddr))
            {
                ...
                {
                    printf(
                        "%s: opening file failed because the file name address 0x%08lX is unmapped\n",
            ...
            {
                printf(
                    "%s: opening file failed because the file name address 0x%08lX is in an invalid segment\n",
                    selfie_name, (uint64_t)vaddr);
...
void implement_openat(uint64_t *context)
{
    ...
    if (down_load_string(context, vfilename, filename_buffer))
    {
        ...
        *(get_regs(context) + REG_A0) = fd;
        ...
        if (debug_open)
            printf(
                "%s: opened file %s with flags 0x%lX and mode 0o%lo returning file descriptor %lu\n",
                selfie_name, filename_buffer, flags, mode, fd);
    }
    else
        *(get_regs(context) + REG_A0) = sign_shrink(-1, SYSCALL_BITWIDTH);
...
void emit_switch()
{
    create_symbol_table_entry(LIBRARY_TABLE, "hypster_switch", 0, PROCEDURE, UINT64STAR_T, 2,
                              code_size);
...
uint64_t cache_tag(uint64_t *cache, uint64_t address) { return address / cache_set_size(cache); }
...
uint64_t *handle_cache_miss(uint64_t *cache, uint64_t *cache_block, uint64_t paddr,
                            uint64_t is_access)
{
    if (is_access)
    {
...
void print_cache_profile(uint64_t hits, uint64_t misses, char *cache_name)
{
    ...
    printf("%lu(%lu.%.2lu%%),%lu(%lu.%.2lu%%)", hits, percentage_format_integral_2(accesses, hits),
...
uint64_t get_page_of_virtual_address(uint64_t vaddr) { return vaddr / PAGESIZE; }

uint64_t get_virtual_address_of_page_start(uint64_t page) { return page * PAGESIZE; }
...
void emit_fetch_stack_pointer()
{
    create_symbol_table_entry(LIBRARY_TABLE, "fetch_stack_pointer", 0, PROCEDURE, UINT64_T, 0,
                              code_size);
...
void emit_fetch_global_pointer()
{
    create_symbol_table_entry(LIBRARY_TABLE, "fetch_global_pointer", 0, PROCEDURE, UINT64_T, 0,
                              code_size);
...
void emit_fetch_data_segment_size_interface()
{
    create_symbol_table_entry(LIBRARY_TABLE, "fetch_data_segment_size", 0, PROCEDURE, UINT64_T, 0,
                              code_size);
...
void implement_gc_brk(uint64_t *context)
{
    ...
    if (program_break > get_program_break(context))
    {
        ...
        *(get_regs(context) + REG_A0) = (uint64_t)gc_malloc_implementation(context, size);
        ...
        store_virtual_memory(get_pt(context), get_data_seg_end_gc(context) - WORDSIZE,
...
void print_gc_profile(uint64_t *context)
{
    printf("%s: gc:      %lu.%.2luMB requested in %lu mallocs (%lu gced, %lu reuses)\n",
           ...
           ratio_format_fractional_2(gc_mem_mallocated, MEGABYTE), gc_num_mallocated,
           gc_num_gced_mallocs, gc_num_reused_mallocs);
    printf("%s: gc:      %lu.%.2luMB(%lu.%.2lu%%) reused in %lu reused mallocs\n", selfie_name,
           ...
           percentage_format_fractional_2(gc_mem_mallocated, gc_mem_reused), gc_num_reused_mallocs);
    printf("%s: gc:      %lu.%.2luMB collected in %lu gc runs\n", selfie_name,
           ratio_format_integral_2(gc_mem_collected, MEGABYTE),
           ratio_format_fractional_2(gc_mem_collected, MEGABYTE), gc_num_collects);
    printf(
        "%s: gc:      %lu.%.2luMB(%lu.%.2lu%%) allocated in %lu mallocs (%lu gced, %lu ungced)\n",
        selfie_name, ratio_format_integral_2(gc_mem_objects + gc_mem_metadata, MEGABYTE),
        ...
        gc_num_gced_mallocs + gc_num_ungced_mallocs, gc_num_gced_mallocs, gc_num_ungced_mallocs);
    printf("%s: gc:      %lu.%.2luMB(%lu.%.2lu%%) allocated in %lu gced mallocs\n", selfie_name,
           ...
           percentage_format_fractional_2(gc_mem_mallocated, gc_mem_objects), gc_num_gced_mallocs);
...
uint64_t print_lui()
{
    return print_code_context_for_instruction(pc) + dprintf(output_fd, "%s %s,0x%lX",
...
void record_lui_addi_add_sub_mul_divu_remu_sltu_jal_jalr() { record_state(*(registers + rd)); }
...
uint64_t print_addi()
{
    ...
    return w + dprintf(output_fd, "%s %s,%s,%ld", get_mnemonic(is), get_register_name(rd),
...
uint64_t print_add_sub_mul_divu_remu_sltu()
{
    return print_code_context_for_instruction(pc) +
           dprintf(output_fd, "%s %s,%s,%s", get_mnemonic(is), get_register_name(rd),
...
void do_xori()
{
    ...
    {
        // semantics of xori
        if (imm == UINT64_MAX) // If imm is -1 then it is not pseudo instruction
        {
...
uint64_t print_load()
{
    return print_code_context_for_instruction(pc) + dprintf(output_fd, "%s %s,%ld(%s)",
                                                            get_mnemonic(is), get_register_name(rd),
                                                            imm, get_register_name(rs1));
}
...
{
    return print_code_context_for_instruction(pc) +
           dprintf(output_fd, "%s %s,%ld(%s)", get_mnemonic(is), get_register_name(rs2), imm,
...
uint64_t print_beq()
{
    ...
    w = print_code_context_for_instruction(pc) +
        dprintf(output_fd, "%s %s,%s,%ld", get_mnemonic(is), get_register_name(rs1),
                get_register_name(rs2), signed_division(imm, INSTRUCTIONSIZE));
...
uint64_t print_jal()
{
    ...
    w = print_code_context_for_instruction(pc) + dprintf(output_fd, "%s %s,%ld", get_mnemonic(is),
...
{
    return print_code_context_for_instruction(pc) +
           dprintf(output_fd, "%s %s,%ld(%s)", get_mnemonic(is), get_register_name(rd),
...
uint64_t print_ecall()
{
    return print_code_context_for_instruction(pc) + dprintf(output_fd, "%s", get_mnemonic(is));
}
...
void selfie_disassemble(uint64_t verbose)
{
    ...
    while (pc < code_size)
    {
        ...
        number_of_written_characters =
            number_of_written_characters + print_instruction() + dprintf(output_fd, "\n");
    ...
    printf(
        "%s: %lu characters of assembly with %lu %lu-bit RISC-U instructions and %lu bytes of data written into %s\n",
        selfie_name, number_of_written_characters, code_size / INSTRUCTIONSIZE, WORDSIZEINBITS,
        data_size, assembly_name);
}
...
void throw_exception(uint64_t exception, uint64_t fault)
{
    if (get_exception(current_context) != EXCEPTION_NOEXCEPTION)
        ...
        {
            printf("%s: context 0x%08lX throws exception: ", selfie_name,
...
void decode()
{
    ...
    { // could be ADD, SUB, MUL, DIVU, REMU, SLTU, SLL, SRL
        decode_r_format();
...
void print_per_instruction_profile(char *message, uint64_t total, uint64_t *counters)
{
    ...
    print_per_instruction_counter(
        total, counters,
        print_per_instruction_counter(total, counters,
...
void print_access_profile(char *message, char *padding, uint64_t reads, uint64_t writes)
{
    if (reads + writes > 0)
    {
        ...
        printf("%s: %s%s%lu,%lu,%lu[%lu.%.2lu]\n", selfie_name, message, padding, reads + writes,
               reads, writes, ratio_format_integral_2(reads, writes),
...
void print_per_register_profile(uint64_t reg)
{
    print_access_profile(get_register_name(reg), " register:   ", *(reads_per_register + reg),
...
void print_profile(uint64_t *context)
{
    ...
    printf("%s:          %lu.%.2luMB allocated in %lu mallocs\n", selfie_name,
           ratio_format_integral_2(mc_brk, MEGABYTE), ratio_format_fractional_2(mc_brk, MEGABYTE),
           sc_brk);
    ...
    printf("%s:          %lu.%.2luMB(%lu.%.2lu%% of %luMB) mapped memory\n", selfie_name,
           ratio_format_integral_2(pused(), MEGABYTE), ratio_format_fractional_2(pused(), MEGABYTE),
...
uint64_t *create_context(uint64_t *parent, uint64_t *vctxt)
{
    ...
    if (debug_create)
        printf("%s: parent context 0x%08lX created child context 0x%08lX\n", selfie_name,
               (uint64_t)parent, (uint64_t)used_contexts);
...
void restore_region(uint64_t *context, uint64_t *table, uint64_t *parent_table, uint64_t lo,
                    uint64_t hi)
{
    uint64_t frame;
    ...
    while (lo < hi)
    {
        if (is_virtual_address_mapped(
                parent_table, (uint64_t)get_PTE_address_for_page(parent_table, table, lo)))
        {
...
void restore_context(uint64_t *context)
{
    ...
    if (get_parent(context) != MY_CONTEXT)
    {
        ...
        set_used_list_head(context,
                           (uint64_t *)load_virtual_memory(parent_table, used_list_head(vctxt)));
        set_free_list_head(context,
                           (uint64_t *)load_virtual_memory(parent_table, free_list_head(vctxt)));
...
uint64_t pused() { return allocated_page_frame_memory - free_page_frame_memory; }
...
uint64_t selfie_run(uint64_t machine)
{
    ...
    printf("%s: selfie terminating %lu-bit RISC-U binary %s with exit code %ld\n", selfie_name,
           WORDSIZEINBITS, get_name(current_context), sign_extend(exit_code, SYSCALL_BITWIDTH));
...
uint64_t selfie(uint64_t extras)
{
    ...
    else
    {
        ...
        printf("%s: %lu-bit unsigned integers and %lu-bit pointers hosted on ", selfie_name,
               SIZEOFUINT64INBITS, SIZEOFUINT64STARINBITS);
        print_host_os();
        println();
...
int main(int argc, char **argv)
{
    ...
    // Personalize my selfie
    printf("%s: This is Guenther Pritz's Selfie!\n", selfie_name);
